...
async function initApp() {
    ...
    // AKTIFKAN FITUR AUTO LOGOUT DI SINI
    setupAutoLogout();
...
async function handleSimpanSurat(event, modul) {
    ...
    try {
        ...
        const payload = {
            ...
            // --- TAMBAHAN KEAMANAN WAJIB ---
            apiKey: "%55Apprest---2026@stone***LPHG",
        ...
        if (modul === 'masuk') {
            ...
            // --- LOGIKA DISPOSISI ---
            let disposisiArr = [];
            const cbSemua = document.getElementById('cb-disposisi-semua');

            // Cek prioritas utama: Apakah opsi "Semua" dicentang?
            if (cbSemua && cbSemua.checked) {
                disposisiArr.push("Semua");
            } else {
                // Jika "Semua" tidak dicentang, baru kita ambil kotak yang dicentang satu per satu
                document.querySelectorAll('.cb-disposisi:checked').forEach(cb => {
                    disposisiArr.push(cb.value);
                });
                
                const cbLainnya = document.getElementById('cb-disposisi-lainnya');
                if (cbLainnya && cbLainnya.checked) {
                    const teksLainnya = document.getElementById('masuk-disposisi-lainnya-text').value.trim();
                    if (teksLainnya) disposisiArr.push(teksLainnya);
                }
            }
            
            payload.keterangan = disposisiArr.length > 0 ? disposisiArr.join(';') : '-';
...
function renderBarisSuratMasuk(dataRows) {
    ...
    let html = `<div class="responsive-table-wrapper"><table class="seamless-table">
        <thead><tr>
            ...
            <th style="width: 28%">Jenis & Perihal</th>
            <th style="width: 18%">Asal</th>
            <th style="width: 22%">Disposisi</th>
    ...
    dataRows.forEach(surat => {
        ...
        // Format Disposisi menjadi Bullet Points
        const dataDisposisi = surat['Disposisi'] || surat['Keterangan']; // Mengantisipasi jika kamu mengubah Header di Sheet
        let disposisiHtml = '<span class="cell-muted-text">-</span>';
        if (dataDisposisi && dataDisposisi !== '-') {
            const arrDisp = dataDisposisi.split(';');
            disposisiHtml = `<ul style="margin: 0 0 0 16px; padding: 0; font-size: 0.85rem; color: #475569;">
                ${arrDisp.map(d => `<li>${d}</li>`).join('')}
            </ul>`;
        }

        html += `<tr>
            <td><div class="cell-stacked"><span class="cell-date">${tglBersih}</span><span class="cell-main-text" style="font-weight: 600;">${surat['Nomor Surat'] || '-'}</span></div></td>
            <td><div class="cell-stacked"><div><span class="badge-jenis-surat badge-masuk">${surat['Jenis Surat'] || 'Surat'}</span></div><span class="cell-sub-text">${surat['Perihal/Deskripsi'] || '-'}</span></div></td>
...
function renderKontrolPagination(modul, totalHalaman, halamanAktif) {
    ...
    // Sembunyikan bar jika data muat dalam 1 halaman
    if (totalHalaman <= 1) {
        pagBar.innerHTML = ''; 
        return;
    }
    ...
    // Tombol Sebelumnya
    let htm = `<button class="btn-page" ${halamanAktif === 1 ? 'disabled' : ''} onclick="pindahKeHalaman('${modul}', ${halamanAktif - 1})">Prev</button>`;

    // --- LOGIKA SMART PAGINATION ---
    
    // Menentukan rentang tombol angka yang muncul di tengah
    const rentang = 1; // Menampilkan 1 angka di kiri dan 1 angka di kanan halaman aktif
    let mulai = Math.max(2, halamanAktif - rentang);
    let akhir = Math.min(totalHalaman - 1, halamanAktif + rentang);

    // 1. Selalu tampilkan halaman pertama (1)
    htm += `<button class="btn-page ${1 === halamanAktif ? 'active' : ''}" onclick="pindahKeHalaman('${modul}', 1)">1</button>`;

    // 2. Tambahkan titik-titik (elipsis) bagian kiri jika ada lompatan angka
    if (mulai > 2) {
        htm += `<button class="btn-page" disabled style="cursor: default; border: none; background: transparent;">...</button>`;
    }

    // 3. Tampilkan angka berurutan di tengah
    for (let i = mulai; i <= akhir; i++) {
        htm += `<button class="btn-page ${i === halamanAktif ? 'active' : ''}" onclick="pindahKeHalaman('${modul}', ${i})">${i}</button>`;
    }

    // 4. Tambahkan titik-titik (elipsis) bagian kanan jika ada lompatan angka
    if (akhir < totalHalaman - 1) {
        htm += `<button class="btn-page" disabled style="cursor: default; border: none; background: transparent;">...</button>`;
    }

    // 5. Selalu tampilkan halaman terakhir
    if (totalHalaman > 1) {
        htm += `<button class="btn-page ${totalHalaman === halamanAktif ? 'active' : ''}" onclick="pindahKeHalaman('${modul}', ${totalHalaman})">${totalHalaman}</button>`;
    }

    // Tombol Selanjutnya
    htm += `<button class="btn-page" ${halamanAktif === totalHalaman ? 'disabled' : ''} onclick="pindahKeHalaman('${modul}', ${halamanAktif + 1})">Next</button>`;
...
function bukaEditMasuk(surat) {
    ...
    // --- RESET CHECKBOX ---
    document.getElementById('cb-disposisi-semua').checked = false;
    document.getElementById('cb-disposisi-lainnya').checked = false;
    document.getElementById('masuk-disposisi-lainnya-text').style.display = 'none';
    document.getElementById('masuk-disposisi-lainnya-text').value = '';
    document.querySelectorAll('.cb-disposisi').forEach(cb => cb.checked = false);

    // --- CENTANG OTOMATIS BERDASARKAN DATA ---
    const dataDisposisi = surat['Disposisi'] || surat['Keterangan'];
    if (dataDisposisi && dataDisposisi !== '-') {
        // Jika data di database tertulis persis "Semua", cukup centang kotak "Semua"
        if (dataDisposisi === "Semua") {
            document.getElementById('cb-disposisi-semua').checked = true;
        } else {
            const arrDisp = dataDisposisi.split(';');
            let teksLainnyaArr = [];

            arrDisp.forEach(val => {
                const cb = document.querySelector(`.cb-disposisi[value="${val}"]`);
                if (cb) {
                    cb.checked = true;
                } else {
                    teksLainnyaArr.push(val); // Masuk kategori "Lainnya"
                }
            });

            if (teksLainnyaArr.length > 0) {
                document.getElementById('cb-disposisi-lainnya').checked = true;
                document.getElementById('masuk-disposisi-lainnya-text').style.display = 'block';
                document.getElementById('masuk-disposisi-lainnya-text').value = teksLainnyaArr.join('; ');
            }
        }
    }
...
function prosesEksporPDF() {
    ...
    if (modulUnduhAktif === 'masuk') {
        ...
        columns = ["No", "Tanggal & Nomor Surat", "Jenis & Perihal", "Asal/Pengirim", "Disposisi", "URL File"];
        rows = dataTerfilter.map((s, index) => {
            const dataDisposisi = s['Disposisi'] || s['Keterangan'];
            const disposisiBullet = (dataDisposisi && dataDisposisi !== '-') 
                ? dataDisposisi.split(';').map(d => `• ${d}`).join('\n') 
                : '-';
        ...
        if (result.status === 'success') {
            ...
            if (submodul === 'admin' && tipeCrud === 'update' && userMasuk && idLama === userMasuk.username) {
                Swal.fire({
                    ...
                    localStorage.removeItem('currentUser'); // Hapus sesi lokal
                    sessionStorage.removeItem('currentUser'); // Hapus sesi tab
...
async function validasiSesiLatarBelakang() {
    // Ambil data dari salah satu tempat penyimpanan
    const sessionData = localStorage.getItem('currentUser') || sessionStorage.getItem('currentUser');
...
// ==========================================
// FITUR AUTO-LOGOUT (SESSION TIMEOUT)
// ==========================================
function setupAutoLogout() {
    const waktuBatasInaktif = 15 * 60 * 1000; // 15 menit (dalam milidetik)
    let timerInaktivitas;

    // Fungsi untuk mereset ulang waktu jika ada pergerakan
    function resetTimer() {
        clearTimeout(timerInaktivitas);
        timerInaktivitas = setTimeout(logoutOtomatis, waktuBatasInaktif);
    }

    // Fungsi yang dieksekusi saat waktu habis
    function logoutOtomatis() {
        // Hapus data sesi login di memori browser
        localStorage.removeItem('currentUser');
        sessionStorage.removeItem('currentUser');
        
        // Tampilkan notifikasi lalu alihkan ke halaman login
        Swal.fire({
            icon: 'info',
            title: 'Sesi Berakhir',
            text: 'Anda telah keluar otomatis karena tidak ada aktivitas selama 15 menit.',
            confirmButtonColor: '#1a365d',
            allowOutsideClick: false,
            allowEscapeKey: false
        }).then(() => {
    ...
    // Daftar pemicu (event) yang menandakan pengguna sedang aktif/bekerja
    const eventAktivitas = ['mousemove', 'mousedown', 'keypress', 'scroll', 'touchstart'];
    
    // Pasang alat pendeteksi ke seluruh halaman
    eventAktivitas.forEach(event => {
        document.addEventListener(event, resetTimer, { passive: true });
    });

    // Mulai jalankan timer perhitungan saat fungsi pertama kali dipanggil
    resetTimer();
}

// ==========================================
// FITUR KONTROL CHECKBOX DISPOSISI
// ==========================================
function toggleDisposisiSemua(checkbox) {
    const checkboxes = document.querySelectorAll('.cb-disposisi');
    const cbLainnya = document.getElementById('cb-disposisi-lainnya');
    const textInput = document.getElementById('masuk-disposisi-lainnya-text');

    if (checkbox.checked) {
        // Jika "Semua" dicentang, hilangkan centang di kotak opsi lain
...
function toggleDisposisiLainnya(checkbox) {
    const textInput = document.getElementById('masuk-disposisi-lainnya-text');
    textInput.style.display = checkbox.checked ? 'block' : 'none';
    if (!checkbox.checked) textInput.value = ''; // Kosongkan jika batal centang
}
